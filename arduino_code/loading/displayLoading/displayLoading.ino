...
void wait(int time) {
  ...
  for (int i = 0; i < time; i++) {
    ...
    if (receivedMsg[0] != '\0') {
      ...
      if (strcmp(receivedMsg, "mvr\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        moveR();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "mvl\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        moveL();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "vid\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        valRfid();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "iid\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        invalRfid();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "frw\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        forwarding();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "dpg\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        dropping();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "dpd\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        dropped();
        isActionHappened = true;
        isDropped = true;
        return;
      } else if (strcmp(receivedMsg, "wfr\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        waitingForRolling();
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "rtr\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        readyToRoll();
        isActionHappened = true;
        isRdyToRoll = true;
...
void moveR() {
  ...
  u8g.firstPage();
  do {
    u8g.drawStr(25, 16, "Moving to"); //12
    u8g.drawStr(8, 36, "RFID Reader");
...
void pos(char arr[]) {
  char * unit = " m\0";
  char * conc = (char * ) malloc(8);
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB14);
  u8g.firstPage();
  if (arr[1] != '0' && arr[0] == '0') //0xx\0
  {
    strncpy(conc, & arr[1], 3);
    strcat(conc, unit);
    do {
      u8g.drawStr(25, 25, "Position:"); //14
      u8g.drawStr(42, 50, conc);
    } while (u8g.nextPage());
  } else if (arr[1] == '0') //00x\0
  {
    strncpy(conc, & arr[2], 2);
    strcat(conc, unit);
    do {
      u8g.drawStr(25, 25, "Position:"); //14
      u8g.drawStr(49, 50, conc);
    } while (u8g.nextPage());
  } else //if(arr[0]=='9') //999\0 (not on the tbl)
  {
    char tmp[5];
    tmp[0] = '-';
    tmp[1] = ' ';
