#include "ssd1306.h"
#include "U8glib.h"
#include <PinChangeInt.h>

#define FORWARD_PIN 2
#define BACK_PIN 3
#define SPECIAL_PIN 4
#define POWER_PIN 5

U8GLIB_SSD1306_128X64 u8g(U8G_I2C_OPT_NONE|U8G_I2C_OPT_DEV_0);  // I2C / TWI 

const uint8_t psiLit [] PROGMEM {

};

const uint8_t logoLit [] PROGMEM {

};

char * receivedRfid = (char * ) malloc(sizeof(char) * 9);
char * receivedMsg = (char * ) malloc(sizeof(char) * 4);
char * receivedThickness = (char * ) malloc(sizeof(char) * 4);
unsigned long startTime = 0;
bool isActionHappened = false;
bool screenSaver = false;

bool isReadyToQCheck = false;
bool isPreparedForRolling = false;
bool isForwarded = false;
bool isRollingEnded = false;
int firstTime = 0;

volatile unsigned long buttonTimer = 0;
volatile boolean firstManualInfo = true;
volatile boolean isManual = false;
volatile boolean displayMsg = false;
volatile boolean displayMsgIsManual = false;

int forward = 2;
int back = 3;
int special = 4;
int power = 5;

void setup() {
  memset(receivedMsg, '\0', sizeof(receivedMsg));
  memset(receivedRfid, '\0', sizeof(receivedRfid));
  memset(receivedThickness, '\0', sizeof(receivedThickness));
  Serial.begin(115200);
  Serial.setTimeout(100);
  ssd1306_128x64_i2c_init();
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.setColorIndex(1); // Instructs the display to draw with a pixel on. 
  u8g.firstPage();
  pinMode(forward, INPUT);
  pinMode(back, INPUT);
  pinMode(special, INPUT);
  pinMode(power, INPUT);
  attachPinChangeInterrupt(FORWARD_PIN, checkButton, RISING);
  attachPinChangeInterrupt(BACK_PIN, checkButton, RISING);
  attachPinChangeInterrupt(SPECIAL_PIN, checkButton, RISING);
  attachPinChangeInterrupt(POWER_PIN, checkButton, RISING);
  delay(1000);
}

void wait(int time) {
  time /= 100;
  for (int i = 0; i < time; i++) {
    if (displayMsg)
      manual(true);
    else if (displayMsgIsManual)
      manual(isManual);

    Serial.readBytes(receivedMsg, 4);

    if (receivedMsg[0] != '\0') {
      //warning only 1 msg
      Serial.write("r\0", 2); //received
      Serial.flush();

      if (strcmp(receivedMsg, "rcd\0") == 0) {
        while (1) {
          Serial.readBytes(receivedRfid, 9);
          delay(20);
          if (receivedRfid[0] != '\0') {
            //warning only 1 msg
            Serial.write("r\0", 2); //received
            Serial.flush();
            memset(receivedMsg, '\0', sizeof(receivedMsg));
            while (Serial.available())
              Serial.read();
            id(receivedRfid);
            isActionHappened = true;
            return;
          }
        }
      } else if (strcmp(receivedMsg, "tns\0") == 0) {
        while (1) {
          Serial.readBytes(receivedThickness, 4);
          delay(20);
          if (receivedThickness[0] != '\0') {
            //warning only 1 msg
            Serial.write("r\0", 2); //received
            Serial.flush();
            memset(receivedMsg, '\0', sizeof(receivedMsg));
            while (Serial.available())
              Serial.read();
            thickness(receivedThickness); //memset receivedThickness in thickness
            isActionHappened = true;
            isPreparedForRolling = true;
            return;
          }
        }
      }

      //warning maybe timer 
      while (Serial.available())
        Serial.read();

      if (strcmp(receivedMsg, "rtq\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        readyToQCheck();
        isActionHappened = true;
        isReadyToQCheck = true;
        return;
      } else if (strcmp(receivedMsg, "wfq\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        waitingForQCheck(); //...
        isActionHappened = true;
        isForwarded = true;
        return;
      } else if (strcmp(receivedMsg, "pfr\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        preparingForRolling(); //...
        isActionHappened = true;
        return;
      } else if (strcmp(receivedMsg, "fwd\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        forwarding(); //...
        isActionHappened = true;
        isRollingEnded = true;
        return;
      } else if (strcmp(receivedMsg, "rdy\0") == 0) {
        memset(receivedMsg, '\0', sizeof(receivedMsg));
        isActionHappened = false;
        screenSaver = true;
        firstTime = 0;
        return;
      } else {
        firstTime++;
        rolling(receivedMsg);
        isActionHappened = true;
        return;
      }
    }
  }
}

void checkButton() {
  if (millis() - buttonTimer > 500) {
    buttonTimer = millis();
    if (digitalRead(forward) == 1) {
      if (firstManualInfo) {
        firstManualInfo = false;
        displayMsg = true;
        isManual = true;
        //manual(true); 
      }
      //Serial.println("FORWARD");
      Serial.write("f\0", 2);
      Serial.flush();
    } else if (digitalRead(back) == 1) {
      if (firstManualInfo) {
        firstManualInfo = false;
        displayMsg = true;
        isManual = true;
        //manual(true); 
      }
      //Serial.println("BACK");
      Serial.write("b\0", 2);
      Serial.flush();
    } else if (digitalRead(special) == 1) {
      if (firstManualInfo) {
        firstManualInfo = false;
        displayMsg = true;
        isManual = true;
        //manual(true); 
      }
      //Serial.println("SPECIAL");
      Serial.write("s\0", 2);
      Serial.flush();
    } else if (digitalRead(power) == 1) {
      //Serial.println("ON/OFF");
      isManual = !isManual;
      displayMsgIsManual = true;
      Serial.write("o\0", 2);
      Serial.flush();
      //manual(isManual);
    }
  }
}

void manual(boolean isManual) {
  displayMsg = false;
  displayMsgIsManual = false;
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  if (isManual) {
    do {
      u8g.drawStr(6, 25, "Manual Mode"); //12
      u8g.drawStr(50, 50, "ON");
    } while (u8g.nextPage());
  } else {
    firstManualInfo = true;
    do {
      u8g.drawStr(6, 25, "Manual Mode"); //12
      u8g.drawStr(45, 50, "OFF");
    } while (u8g.nextPage());
  }
  delay(1000);
}

bool stopAnim() {
  if (isReadyToQCheck || isPreparedForRolling || isForwarded || isRollingEnded) {
    isReadyToQCheck = false;
    isPreparedForRolling = false;
    isForwarded = false;
    isRollingEnded = false;
    return true;
  } else
    return false;
}

void animation() {
  while (1) {
    for (int i = 48; i < 55; i++) {
      ssd1306_putPixels(i, 49, 0xFF);
    }
    wait(200);
    if (stopAnim()) {
      break;
    }
    for (int i = 60; i < 67; i++) {
      ssd1306_putPixels(i, 49, 0xFF);
    }
    wait(200);
    if (stopAnim()) {
      break;
    }
    for (int i = 72; i < 79; i++) {
      ssd1306_putPixels(i, 49, 0xFF);
    }
    wait(200);
    if (stopAnim()) {
      break;
    }
    ssd1306_clearBlock(48, 49 / 8, 7, 8);
    ssd1306_clearBlock(60, 49 / 8, 7, 8);
    ssd1306_clearBlock(72, 49 / 8, 7, 8);
    wait(200);
    if (stopAnim())
      break;
  }
}

void thickness(char thnss[]) {
  //char* unit = " mm\0";
  //char* conc = (char*) malloc(15);
  //memset(conc, '\0', sizeof(conc));
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  if (thnss[1] == '0') //0x\0
  {
    //char tempChar = thnss[1];
    //strcpy(conc, &tempChar);
    //strcat(conc, thnss);
    //Serial.println();
    //Serial.println(thnss[2]);
    char arrChar[5];
    arrChar[0] = thnss[2];
    arrChar[1] = ' ';
    arrChar[2] = 'm';
    arrChar[3] = 'm';
    arrChar[4] = '\0';
    //strcat(conc, unit); 
    do {
      u8g.drawStr(20, 16, "Thickness"); //12
      u8g.drawStr(40, 36, arrChar);
      u8g.drawStr(27, 56, "received");
    } while (u8g.nextPage());
  } else //xx\0
  {
    //char tempChar0 = thnss[0];
    //char tempChar1 = thnss[1];
    //strcpy(conc, &tempChar0);
    //strcpy(conc, &tempChar1);
    //strcat(conc, unit); 
    char arrChar[6];
    //Serial.println();
    //Serial.println(thnss[1]);
    //Serial.println(thnss[2]);
    arrChar[0] = thnss[1];
    arrChar[1] = thnss[2];
    arrChar[2] = ' ';
    arrChar[3] = 'm';
    arrChar[4] = 'm';
    arrChar[5] = '\0';
    do {
      u8g.drawStr(20, 16, "Thickness"); //12
      u8g.drawStr(35, 36, arrChar);
      u8g.drawStr(27, 56, "received");
    } while (u8g.nextPage());
  }
  memset(receivedThickness, '\0', sizeof(receivedThickness));
  //free(conc);
}
void rolling(char times[]) {
  char * startLine = (char * ) malloc(sizeof(char) * 12);
  strcpy(startLine, "times: \0");
  char * conc = (char * ) malloc(sizeof(char) * 20);
  char tms[2];
  tms[0] = times[0];
  tms[1] = '\0';
  strcpy(conc, startLine);
  strcat(conc, & tms[0]);
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  do {
    u8g.drawStr(33, 13, "Rolling"); //12
    u8g.drawStr(33, 33, conc);
  } while (u8g.nextPage());
  memset(receivedMsg, '\0', sizeof(receivedMsg));
  free(conc);
  free(startLine);
  if (firstTime == 1)
    animation();
}
void readyToQCheck() {
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  do {
    u8g.drawStr(25, 25, "Ready to"); //12
    u8g.drawStr(8, 50, "quality check");
  } while (u8g.nextPage());
}
void waitingForQCheck() {

  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  do {
    u8g.drawStr(18, 13, "Waiting for"); //12
    u8g.drawStr(10, 33, "quality check");
  } while (u8g.nextPage());
  animation();

}

void preparingForRolling() {
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  do {
    u8g.drawStr(22, 13, "Preparing"); //12
    u8g.drawStr(22, 33, "for rolling");
  } while (u8g.nextPage());
  animation();
}

void id(char rfidArr[]) {
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB12);
  u8g.firstPage();
  do {
    u8g.drawStr(52, 16, "ID"); //12
    u8g.drawStr(23, 36, rfidArr);
    u8g.drawStr(25, 56, "received");
  } while (u8g.nextPage());
  memset(receivedRfid, '\0', sizeof(receivedRfid));
}

void forwarding() {
  ssd1306_fillScreen(0x00);
  u8g.setFont(u8g_font_ncenB14);
  u8g.firstPage();
  do {
    u8g.drawStr(8, 30, "Forwarding"); //14
  } while (u8g.nextPage());

  animation();
}

void mainCheck() {
  if (isActionHappened) {
    screenSaver = false;
    while (1) {
      isActionHappened = false;
      startTime = millis();
      while (millis() - startTime < 300000 && isActionHappened == false && screenSaver == false)
      //5 min waiting then screensaver
      {
        wait(10000);
      }
      if (!isActionHappened) {
        ssd1306_fillScreen(0x00);
        break;
      }

    }
  }
}

void loop() {
  //Serial.println();
  //Serial.println(freeRam());
  //Serial.println();
  if (!isManual) {
    ssd1306_fillScreen(0x00);
    ssd1306_drawBitmap(25, 1, 80, 50, psiLit);
  }
  wait(2000);
  mainCheck();
  if (!isManual) {
    ssd1306_fillScreen(0x00);
    ssd1306_drawBitmap(25, 1, 80, 50, logoLit);
  }
  wait(2000);
  mainCheck();
  if (!isManual) {
    ssd1306_fillScreen(0x00);
    u8g.setFont(u8g_font_ncenB12);
    u8g.firstPage();
    do {
      u8g.drawStr(17, 25, "Waiting for"); //12
      u8g.drawStr(9, 50, "new material");
    } while (u8g.nextPage());
  }
  wait(2000);
  mainCheck();

}

int freeRam() {
  extern int __heap_start, * __brkval;
  int v;
  return (int) & v - (__brkval == 0 ? (int) & __heap_start : (int) __brkval);
}

//u8g.drawStr( 17, 25, "Waiting for");//12
//u8g.drawStr( 9, 50, "new material");

//u8g.drawStr( 25, 25, "Material");//14
//u8g.drawStr( 23, 50, "detected");

//u8g.drawStr( 8, 16, "Please, move");//12
//u8g.drawStr( 25, 36, "material");
//u8g.drawStr( 30, 56, "right -->");

//u8g.drawStr( 8, 16, "Moving to");//12
//u8g.drawStr( 5, 36, "RFID Reader");
//u8g.drawStr( 33, 56, "<-- left");

//u8g.drawStr( 40, 25, "RFID");//14
//u8g.drawStr( 23, 50, "674516f2");

//u8g.drawStr( 40, 25, "Valid");//14
//u8g.drawStr( 39, 50, "RFID");

//u8g.drawStr( 32, 25, "Invalid");//14
//u8g.drawStr( 39, 50, "RFID");

//u8g.drawStr( 8, 40, "Forwarding");//14

//u8g.drawStr( 20, 30, "Dropping");//14

//u8g.drawStr( 23, 40, "Dropped");//14

//u8g.drawStr( 25, 25, "Position:");//14
//u8g.drawStr( 39, 50, "20 cm");
//u8g.drawStr( 46, 50, "2 cm");

//u8g.drawStr( 18, 13, "Waiting for");//12
//u8g.drawStr( 35, 33, "rolling");

//u8g.drawStr( 22, 25, "Ready to");//14
//u8g.drawStr( 48, 50, "roll");

//u8g.drawStr( 7, 25, "Manual mode");//12
//u8g.drawStr( 47, 50, "OFF");